...
async def insert_user(user: UserIn = None):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="richiesta fallita")
    return {"accountId": id}
...
async def transfer_money(transfer: TransactionIn):
    res = await transaction(transfer.sender, transfer.receiver, transfer.amount)
    if (res == 400):
...
async def check_exists(accountId):
    check_query = users.select().where(users.c.accountId == accountId)
    check = await database.fetch_one(check_query)
...
async def transaction(sender: str, receiver: str, amount: float):
    if(not(await check_exists(sender)) or not(await check_exists(receiver))):
        return 400
